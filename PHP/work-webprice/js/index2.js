...
(function (a) {
	...
	a.Autocompleter.Select = function (e, j, l, p) {
		...
		function n() {
			...
			if(a("#"+e.resultsClass).length>0){
				sc=a("#"+e.resultsClass).hide();
				if(sc.find("#scroller").length>0){
					c=sc.find("#scroller");
				}else{
					c=a('<div id="scroller"/>').appendTo(sc).hide().addClass("scroller");
				}
			}else{
				c = a("<div/>").hide().addClass(e.resultsClass).css("position", "absolute").appendTo(e.parentContainer);
			}
			c.empty();
		...
		return {
			...
			show : function () {
				...
				sc.css({
					...
					top : e.top,
					left : e.left,
					height : e.height > 0 ? e.height - e.top : a(window).height() - e.top
				}).show();
				if (e.scroll) {
					o.scrollTop(0);
...
!function (a, b) {
	var myScroll = {
		refreshP : function(){
			$("#priceInfo").css({
				height : b.windowH.height - $("#se-bar").height() - $(".banners").height() - $(".product").height() - $(".TabTitle").height() - 15
			});
			//重置选型页面区域大小
...
(window, window.lib || (window.lib = {})), function (a, b) {
	...
	var search = {
			...
			$.ajax({
				...
				success : function (e) {
					if(e==null || e.Table.length==0){
						...
						var scrollH = b.windowH.height - $("#ser-bar").height();
						var contentH=$("#e-content").height();
						if(contentH>scrollH){
							$("#epullUp").show();
						}
					}
		...
		showSeries : function (){
			...
			$("#searchser").css({
				height : b.windowH.height - $("#ssr-bar").height()
			});
			$("#sresult").show(),
			$("#srTop").show(),
			$("#srTop").css({
		...
		autocompleteClick : function(event, rdata, factoryid){
			//厂家搜索点击事件,显示系列页面
			$("#sf-text").blur();
			b.SeriesTree.init(rdata.name,rdata.fid);
			//加入历史搜索记录
			storage.addHistory("factory",rdata.fid,rdata.name);
		},
		refresh : function(){
			//厂家搜索初始化滚动条
...
!function (a, b) {
	...
	//搜索历史
	myScroll.myHScroll = new iScroll('history');
	myScroll.myTScroll = new iScroll('sub-tab');
	myScroll.myAScroll = new iScroll('ac_results');
